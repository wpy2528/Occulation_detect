...
parser.add_argument('--checkpoint',
                    default="output/onnx/20250610-175033-resnet18-224.onnx",
...
def img_dataset(args, img_path, onnx_session, size=224, score=0, show=False):
    imgs = load_imgs(img_path)
    if args.output_dir:
        true_path = os.path.join(args.output_dir, "True")
        os.makedirs(true_path, exist_ok=True)
        false_path = os.path.join(args.output_dir, "False")
    ...
    if isinstance(imgs, list):
        ...
        for root, img_file in imgs:
            ...
            print((
                    f"img:{os.path.basename(img_file)}" + '\t' + f"label:{label}" + '\t' + f"pred:{result}" + '\t' + f"score:{np.max(pred) * 100:.2f}%").expandtabs(
                40))
            if label_index == pred_index and np.max(pred) > score:
                tp += 1
                cls_tp_dict[label] += 1
                ...
                if args.output_dir:
                    if os.path.exists(args.output_dir):
                        save_image(img, pred_index, label, np.max(pred), os.path.basename(img_file), save_path=true_path)
            elif np.max(pred) < score:
                continue
            else:
                fp += 1
                cls_truth_sum_dict[label] += 1
                ...
                if args.output_dir:
                    if os.path.exists(args.output_dir):
                        save_image(img, pred_index, label, np.max(pred), os.path.basename(img_file), save_path=false_path)
                if show:
...
def main():
    ...
    img_dataset(args, args.input_file, onnx_session, size=args.img_size, score=args.score, show=False)
    caculate()
