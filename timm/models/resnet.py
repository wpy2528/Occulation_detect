...
class ChannelAttentionModule(nn.Module):
    def __init__(self, c1, ratio=16):
        ...
        mid_channel = c1 // ratio
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.f1 = nn.Conv2d(c1, mid_channel, 1, bias=False)
        self.relu = nn.ReLU()
        self.f2 = nn.Conv2d(mid_channel, c1, 1, bias=False)
...
class CBAM(nn.Module):
    def __init__(self, c1, ratio=16, spatial_kernel=7):
        super(CBAM, self).__init__()
        self.channel_attention = ChannelAttentionModule(c1, ratio)
        self.spatial_attention = SpatialAttentionModule(spatial_kernel)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_DWconv and use_GSconv == 0 and not use_DSconv:
            self.conv1 = DepthWiseSeparableConv(inplanes, first_planes, k_size=3, s=1 if use_aa else stride,
                                                p=first_dilation)
            self.conv2 = DepthWiseSeparableConv(first_planes, outplanes, k_size=3, p=dilation)
        elif use_DWconv and use_GSconv != 0 or use_DSconv and use_GSconv != 0:
            self.conv1 = GroupWiseSeparableConv(inplanes, first_planes, k_size=3, s=1 if use_aa else stride,
                                                p=first_dilation, groups=use_GSconv)
            self.conv2 = GroupWiseSeparableConv(first_planes, outplanes, k_size=3, p=dilation, groups=use_GSconv)
        elif use_DWconv and use_DSconv:
            # self.conv1 = DSConv2D(inplanes, first_planes, k=3, s=1 if use_aa else stride, p=first_dilation)
            # # self.conv2 = DSConv2D(first_planes, outplanes, k=3, p=dilation)
            # self.conv2 = DepthWiseSeparableConv(first_planes, outplanes, k_size=3, p=dilation)
            self.conv1 = DepthWiseSeparableConv(inplanes, first_planes, k_size=3, s=1 if use_aa else stride,
                                                p=first_dilation)
            self.conv2 = DSConv2D(first_planes, outplanes, k=3, p=dilation)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if use_DWconv:
            self.conv1 = DepthWiseSeparableConv(inplanes, first_planes, k_size=1)
            self.conv2 = DepthWiseSeparableConv(first_planes, width, k_size=3, s=1 if use_aa else stride,
                                                p=first_dilation)
            self.conv3 = DepthWiseSeparableConv(width, outplanes, k_size=1)
        elif use_GSconv != 1:
            self.conv1 = GroupWiseSeparableConv(inplanes, first_planes, k_size=1, groups=use_GSconv)
            self.conv2 = GroupWiseSeparableConv(first_planes, width, k_size=3, s=1 if use_aa else stride,
                                                p=first_dilation, groups=use_GSconv)
            self.conv3 = GroupWiseSeparableConv(width, outplanes, k_size=1, groups=use_GSconv)
        # elif not use_DWconv and use_DSconv:
        #     self.conv1 = DSConv2D(inplanes, first_planes, k=1)
        #     self.conv2 = DSConv2D(first_planes, width, k=3, s=1 if use_aa else stride,
        #                           p=first_dilation)
        #     self.conv3 = DSConv2D(width, outplanes, k=1)
        else:
...
def get_planes_from_basic_block(block):
    # 遍历 BasicBlock 中的每一层
    ...
    return None  # 如果没有找到 DepthWiseSeparableConv，返回 None
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.use_ppm_last_block:
            for i, stage in enumerate(stage_modules):
                if isinstance(stage, tuple):  # 如果是 tuple，先转换为 nn.ModuleList
                    stage = nn.Sequential(*[m for m in stage if isinstance(m, nn.Module)])

                if i == 3 and self.use_ppm_last_block:  # 仅在最后一层（layer4）使用 PPM
                    last_block = stage[-1]

                    for layer in last_block:
                        # 遍历 last_block (一个 Sequential) 中的层，找到 Conv2d 层并获取输出通道数
                        planes = get_planes_from_basic_block(layer)  # 获取 planes

                    # 如果找到了 planes，加入 PyramidPoolingModule
                    if planes is not None:
                        ppm = PyramidPoolingModule(planes, planes)
                        stage[-1] = nn.Sequential(*list(stage[-1]) + [ppm])  # 将 PPM 加到最后一层

                    # 将修改后的 stage 添加到模型中
                self.add_module(f'layer{i + 1}', stage)
        else:
            for stage in stage_modules:
                self.add_module(*stage)  # layer1, layer2, etc

        if use_SAFM:
            self.safm = SAFM(dim=512, n_levels=2)
...
def resnet18(pretrained=False, **kwargs) -> ResNet:
    ...
    """
    keys_to_block = ['attn_layer', 'use_CBAM', 'use_DWconv', 'use_GSconv', 'use_DSconv']
    block_args = {key: kwargs[key] for key in keys_to_block if key in kwargs}
    model_args = dict(block=BasicBlock, layers=[2, 2, 2, 2], block_args=block_args)
